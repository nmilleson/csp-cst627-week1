"""Plot the scikit-digits double-descent width sweep: mean train/test error vs. hidden width."""
...
IN_CSV = "double_descent_scikit_results.csv"
OUT_PNG = "double_descent_scikit_curve.png"
...
def main():
    ...
    # interpolation threshold = first width at which the model fits the (noisy)
    # training set essentially perfectly
    threshold_width = next(w for w, e in zip(widths, train_mean) if e <= 1e-6)
    ...
    ax.axvline(x=threshold_width, color="#555555", linestyle="--", linewidth=1.5,
               zorder=1, label=f"interpolation threshold (width={threshold_width})")
    ...
    ax.set_title("Model-wise double descent: sklearn digits (25% label noise)")
